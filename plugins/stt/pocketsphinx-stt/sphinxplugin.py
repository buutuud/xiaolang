...
class PocketsphinxSTTPlugin(plugin.STTPlugin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        lm_path = settings.data('model', 'sphinx.lm')
        dict_path = settings.data('model', 'sphinx.dic')

        default_dir = "/usr/local/share/pocketsphinx/model/hmm/en_US/hub4wsj_sc_8k"
        hmm_dir = self.profile.get('PocketSphinx', 'HmmDir', default_dir)
    ...
    def transcribe(self, fp):
        ...
        words = result.decode('utf8')
        transcribed = [words] if result != '' else []
        self._logger.info('Transcribed: [%s]', words)
        return transcribed
